...
def get_entry_prevclose_todayvol(df_daily, ticker):
    ...
    CLOSED: entry = official daily close; entry_ts = 16:00 ET.
    """
    now, open_t, close_t = _market_session_state()
    et_today = now.date()
...
def evaluate_ticker(
    ...
    if rr_to_res < RR_MIN_DEFAULT:  # use eval arg rr_min if you pass it differently
        if rr_to_res < rr_min:
            return None, "rr_to_res_below_min"
...
def run_scan(
    ...
    for t in tickers:
        ...
        if reason is None:
            # Always attach options columns — even if quotes fail, we emit empty fields
            opt_block = {
            ...
            if with_options:
                ...
                if err is None and opt:
                    opt_block.update(opt)
                else:
                    opt_block["PricingNote"] = (err or "")
            row.update(opt_block)
            results.append(row)
    ...
    cols = base_cols + opt_cols  # <- always include
...
def main():
    ...
    run_scan(
        ...
        rr_min=args.rr_min,
        stop_mode=args.stop_mode,
        with_options=args.with_options or True,
